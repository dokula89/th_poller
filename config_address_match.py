...
from config_core import *
    ...
    # Fetch state from major_metros via queue_websites.major_metro_id
    network_state = None
    try:
        ...
        if row and row.get('state'):
            network_state = row['state']
            log_to_file(f"[Address Match] Fetched state for job {job_id}: {network_state}")
        cursor.close()
        temp_conn.close()
    except Exception as e:
        log_to_file(f"[Address Match] Failed to fetch state from major_metros: {e}")
    ...
    # Position window: centered horizontally, 10% from top, ensure fully visible
    ...
    # Set window size (80% of screen width, 85% of screen height to ensure it fits)
    ...
    def run_for_address(row_id, google_address, preloaded_ga_id=None):
        ...
        try:
            ...
            # Add state parameter if available to reduce false positives
            state_param = f"&state={requests.utils.quote(network_state)}" if network_state else ""
            
            if place_id_to_use:
                encoded_place_id = requests.utils.quote(place_id_to_use)
                url = php_url(f"step5/find_or_create_place.php?place_id={encoded_place_id}&html=yes&debug=1{state_param}")
                log_to_file(f"[Address Match] Using place_id lookup: {place_id_to_use} with state: {network_state}")
            else:
                encoded_address = requests.utils.quote(google_address)
                url = php_url(f"step5/find_or_create_place.php?address={encoded_address}&html=yes&debug=1{state_param}")
                log_to_file(f"[Address Match] Searching address: {google_address} with state: {network_state}")
            ...
            try:
                ...
                # Minimum threshold: 95% similarity (adjust as needed)
    ...
    def update_job_status_to_done():
        """Update the queue_websites job status to 'done' when all address matches are completed."""
        try:
            import requests
            ...
            if response.status_code == 200:
                log_to_file(f"[Address Match] Job {job_id} marked as 'done' in queue_websites")
                print(f"[Address Match] Job {job_id} marked as 'done'")
            else:
                log_to_file(f"[Address Match] Failed to update status: {response.status_code}")
        except Exception as e:
    ...
    def process_next_row():
        ...
        if current_idx >= len(all_rows):
            ...
            if not has_empty_pregaid:
                ...
                # Close the Address Match window after marking done
                try:
                    window.destroy()
                except Exception:
                    pass
